...
`docs/architecture/KODER_BACKEND_ROADMAP.md` wherever they conflict with the decisions below.
...
- `docs/architecture/KODER_BACKEND_ROADMAP.md` - mark the old Redis-authoritative scoring text as
  superseded and link this Phase 7 decision.

### Tests to create or modify

- `backend/tests/leaderboard/test_leaderboard_encoding.js`
- `backend/tests/leaderboard/test_leaderboard_projection_integration.js`
- `backend/tests/leaderboard/test_leaderboard_rebuild.js`
- No separate leaderboard failure-modes test file exists; failure and recovery scenarios are covered by the projection, rebuild, standings, finalization, and regression tests listed below.
- `backend/tests/leaderboard/test_standings_api.js` - Redis path, Mongo fallback, competition ranks,
  and finalized snapshot behavior.
- `backend/tests/contest/test_contest_finalization.js` - Redis unavailable during finalization and
  finalized reads bypassing Redis.
- `backend/tests/scoring/test_scoring_ordering.js` - projection equality after all arrival-order
  permutations, without moving scoring authority to Redis.

### Documentation to update

- `docs/phases/phase-07/REDIS_LEADERBOARD.md` - implementation decisions and operational runbook.
- `docs/architecture/KODER_BACKEND_ROADMAP.md` - corrected source-of-truth boundary and issue status.
...
The focused suite is `tests/regression/test_issue706_integration.js`. It uses the real
...
- **Focused test:** `tests/regression/test_issue706_integration.js` — 11 scenarios passed.
...
- `backend/tests/leaderboard/test_standings_api.js`
- `backend/tests/contest/test_contest_finalization.js`
- `backend/tests/scoring/test_scoring_ordering.js`
- `backend/tests/scoring/test_scoring_reconciliation.js`
- `workers/common/executionEngine.js`
