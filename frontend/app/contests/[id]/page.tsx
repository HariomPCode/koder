...
export default function ContestDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setLoadError(
        caught instanceof Error ? caught.message : "Unable to load contest.",
      );
    }
  }, [contestId]);
  ...
  if (loadError)
    return (
      <PageContainer>
        <ErrorState title="Unable to load contest" description={loadError} />
      </PageContainer>
    );
  if (!contest)
    return (
  ...
  const toggleRegistration = async () => {
    ...
    try {
      ...
      setRegistered(
        Boolean(
          "registered" in response ? response.registered : !response.removed,
        ),
      );
    } catch (caught) {
      setActionError(
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem]">
        <section className="rounded-xl bg-card p-6 ring-1 ring-foreground/10">
          ...
          <dl className="mt-5 grid gap-4 sm:grid-cols-3">
            <div>
              <dt className="text-xs text-muted-foreground">Problems</dt>
        ...
        <div className="space-y-4">
          {status === "authenticated" ? (
            <>
              <Button
                disabled={pending || contest.status !== "REGISTRATION"}
                onClick={() => void toggleRegistration()}
                className="w-full"
              >
                {pending ? "Updating..." : registered ? "Unregister" : "Register"}
              </Button>
              {actionError ? (
          ...
          {standing ? <MyStanding standing={standing} /> : null}
        </div>
      </div>
      <nav className="mt-8 flex flex-wrap gap-4 text-sm font-semibold underline underline-offset-4">
        <Link href={`/contests/${contest._id}/leaderboard`}>Leaderboard</Link>
        <Link href={`/contests/${contest._id}/submissions`}>
          My submissions
        </Link>
        {registered ? (
          <Link href={`/contests/${contest._id}/problems`}>
            Contest problems
          </Link>
        ) : null}
      </nav>
