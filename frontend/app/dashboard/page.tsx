...
export default function Dashboard() {
  ...
  return <main className="mx-auto w-full max-w-7xl p-6 sm:p-8">
    ...
    <section className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.55fr)_minmax(18rem,1fr)]">
      <Card><CardContent className="p-6"><h2 className="text-lg font-semibold text-foreground">Recent Submissions</h2>{stats.recentSubmissions.length ? <><div className="mt-4 hidden grid-cols-[minmax(0,1fr)_5rem_7.5rem_4.5rem_4.5rem] gap-3 border-b pb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground md:grid"><span>Problem</span><span>Language</span><span>Verdict</span><span>Runtime</span><span>When</span></div><div className="divide-y">{stats.recentSubmissions.map((submission, index) => <div key={`${submission.question.slug}-${submission.createdAt}-${index}`} className="py-3"><div className="hidden grid-cols-[minmax(0,1fr)_5rem_7.5rem_4.5rem_4.5rem] items-center gap-3 md:grid"><Link href={`/problems/${submission.question.slug}`} className="truncate text-sm font-medium text-foreground hover:underline">{submission.question.title}</Link><span className="text-sm text-muted-foreground">{submission.language}</span><span><span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ring-1 ring-inset ${VERDICT_PRESENTATION[submission.verdict]?.className ?? VERDICT_PRESENTATION.pending.className}`}>{submission.verdict}</span></span><span className="text-sm text-muted-foreground">{submission.maxRuntime ? `${submission.maxRuntime} ms` : "—"}</span><span className="text-sm text-muted-foreground">{relativeTime(submission.createdAt)}</span></div><div className="md:hidden"><div className="flex items-start justify-between gap-3"><Link href={`/problems/${submission.question.slug}`} className="text-sm font-medium text-foreground hover:underline">{submission.question.title}</Link><span className={`shrink-0 rounded-full px-2 py-1 text-xs font-medium ring-1 ring-inset ${VERDICT_PRESENTATION[submission.verdict]?.className ?? VERDICT_PRESENTATION.pending.className}`}>{submission.verdict}</span></div><p className="mt-1 text-xs text-muted-foreground">{submission.language} · {submission.maxRuntime ? `${submission.maxRuntime} ms · ` : ""}{relativeTime(submission.createdAt)}</p></div></div>)}</div></> : <p className="mt-4 text-sm text-muted-foreground">Submit your first solution to start tracking your progress.</p>}</CardContent></Card>
